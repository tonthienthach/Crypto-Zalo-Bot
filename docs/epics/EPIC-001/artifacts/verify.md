...
**Status:** Draft — **revised twice on 2026-09-21**: first after commit
`41e4e93` closed 3 of the 5 originally-untested criteria (test-suite level),
then after a scratch Postgres run (this environment has Docker; see §2's
`AC04`/`AC08` rows) closed the remaining 2 with real database execution.
**Created:** 2026-09-21
...
**Overall: pass** — all 10 acceptance criteria pass with captured evidence.
`AC04` and `AC08` were the last two `untested` (both required a real
Postgres instance): closed by spinning up a scratch `postgres:16-alpine`
container via Docker (available in this environment), applying the real
migration (`db/migrations/0001_create_subscribers.sql`), and running the
exact SQL statements `SubscribersService.subscribe()` and
`scripts/db-seed-digest-subscriber.js` issue, twice each, against it. The
container was removed immediately after capturing output — no persistent
...
| `EPIC-001-AC04` | Re-`/dangky` after `/huy` reactivates, no duplicate | pass | Mocked-driver proof: `subscribers.service.spec.ts` "reactivates an existing..." (commit `41e4e93`). **Real-DB proof (2026-09-21, scratch `postgres:16-alpine` container):** insert `chat-real-1` (`watchlist={btc,eth}`, `is_active=t`) → `UPDATE ... is_active=false` (simulating `/huy`) → confirmed `is_active=f` → re-run the exact `INSERT ... ON CONFLICT (chat_id) DO UPDATE` with a new watchlist (`{sol}`) → row reactivated (`is_active=t`, `watchlist={sol}`) → `SELECT count(*) WHERE chat_id='chat-real-1'` → `1` (no duplicate row) |
...
| `EPIC-001-AC08` | `db:seed-digest-subscriber` idempotent (no duplicate on re-run) | pass | **Real-DB proof (2026-09-21, same scratch container):** ran `scripts/db-seed-digest-subscriber.js`'s exact `INSERT INTO subscribers ... ON CONFLICT (chat_id) DO UPDATE ...` statement for `legacy-digest-chat` twice in a row, both succeeding (`INSERT 0 1` each time — Postgres's own confirmation the upsert path was taken cleanly) → `SELECT count(*) WHERE chat_id='legacy-digest-chat'` → `1` (no duplicate, no error) |
...
All 10 now have execution-level evidence (test run or real-DB output) —
none remain on code-read-only evidence as of the second revision.
...
|---|---|---|---|
| 1 | **Fully resolved** (originally: 5 of 10 acceptance criteria `untested`). Revision 1 (`41e4e93`) closed 3 via new test suites. Revision 2 (2026-09-21, scratch Docker Postgres) closed the last 2 (`AC04`, `AC08`) with real database execution — see §2. No outstanding coverage gap remains from this finding. | resolved | `db/migrations/`, `src/subscribers/subscribers.service.ts`, `scripts/db-seed-digest-subscriber.js` |
...
N/A — verdict is `pass`, nothing remains open.
...
4. ~~`AC04`/`AC08` real-DB proof.~~ Done, 2026-09-21 — scratch `postgres:16-alpine` container via Docker, migration applied, both upsert paths exercised twice each, container removed after capturing output. See §2.

**Note for future epics:** this environment has Docker available, so "no
scratch Postgres" should not recur as a verify-blocking gap — reach for
`docker run postgres:16-alpine` (or equivalent) before accepting a
real-DB acceptance criterion as `untested`.
